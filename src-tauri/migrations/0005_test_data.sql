...
INSERT INTO INVOICE_ITEMS (ID, INVOICE_ID, DESCRIPTION, QUANTITY, UNIT_PRICE, LINE_TOTAL)
VALUES (2, 1, 'Hemograma completo con lectura de lámina', 1, 60000.00, 60000.00);

-- Actualizar los generadores de IDs en Firebird
SET TERM ^ ;
EXECUTE BLOCK AS
BEGIN
    PERFORM GEN_ID(GEN_OWNERS_ID, 10);
    PERFORM GEN_ID(GEN_PATIENTS_ID, 10);
    PERFORM GEN_ID(GEN_CONSULTATIONS_ID, 10);
    PERFORM GEN_ID(GEN_SAMPLES_ID, 10);
    PERFORM GEN_ID(GEN_LAB_RESULTS_ID, 10);
    PERFORM GEN_ID(GEN_SURGERIES_ID, 10);
    PERFORM GEN_ID(GEN_INVOICES_ID, 10);
    PERFORM GEN_ID(GEN_INVOICE_ITEMS_ID, 10);
END^
SET TERM ; ^
